...
class TSDF:
    ...
    def integrate_tsdf(
        ...
        sampled_depth = F.grid_sample(
            input=depth_images, grid=grid, mode="bilinear", padding_mode="zeros", align_corners=True
        )  # [batch, N, 1]
        ...
        if color_images is not None:
            sampled_colors = F.grid_sample(
                input=color_images, grid=grid, mode="bilinear", padding_mode="zeros", align_corners=True
            )  # [batch, N, 3]
...
def export_tsdf_mesh(
    ...
    if not use_bounding_box:
        curr_means = pipeline.model.gauss_params["means"].cpu().detach()
        bounding_box_min = curr_means.min(0)[0]
        bounding_box_max = curr_means.max(0)[0]
        aabb = torch.tensor([tuple(bounding_box_min), tuple(bounding_box_max)])
    else:
    ...
    color_images, depth_images, _, _, _ = render_trajectory(
        pipeline,
        cameras,

...
@dataclass
class TwodgsModelConfig(ModelConfig):
    """Splatfacto Model Config, nerfstudio's implementation of Gaussian Splatting"""

    _target: Type = field(default_factory=lambda: TwodgsModel)
    ...
    """Iteration to start distortion loss"""

class TwodgsModel(Model):
    """Nerfstudio's implementation of Gaussian Splatting

    Args:
        config: 2DGS configuration to instantiate model
    """

    config: TwodgsModelConfig
    ...
    def get_outputs(self, camera: Cameras, rgb_only: bool = False) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        render, alpha, normals, normals_from_depth, render_distort, render_median, self.info = rasterization_2dgs(
            means=means_crop,
            quats=quats_crop / quats_crop.norm(dim=-1, keepdim=True),  # rasterization_2dgs does not do normalization internally
            ...
            distloss=self.config.use_dist_loss,
            # rasterize_mode=self.config.rasterize_mode, NOT SUPPORTED FOR 2DGS
            # set some threshold to disregrad small gaussians for faster rendering.
            # radius_clip=3.0,
        )
        
        # import pdb; pdb.set_trace()
        # normalized_quats = quats_crop / quats_crop.norm(dim=-1, keepdim=True)
        # rotation_matrices = quat_to_rotmat(normalized_quats)
        # world_space_normals = rotation_matrices[..., :, 2]  # Shape: [N, 3]
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        ...
        if self.config.use_normal_loss:
            ...
            normals_from_depth *= outputs['accumulation'].detach()
        ...
        if self.config.use_dist_loss:
            if self.step > self.config.dist_start_iter:
                curr_dist_lambda = self.config.dist_lambda
            else:
                curr_dist_lambda = 0.0
            render_distort = outputs["distort"]
            distloss = render_distort.mean()
            distloss = curr_dist_lambda * distloss
        else:
            distloss = torch.tensor(0.0).to(self.device)
